...
def compressed_safetensors_store(temp_dir):
    ...
    return SafetensorsStore(
        str(temp_dir / "compressed_storage"), 
...
class TestSafetensorsIntegration:
    ...
    def test_repository_integration(self, repository_with_safetensors, sample_weights, temp_dir):
    ...
    def test_official_safetensors_compatibility(self, official_safetensors_file, temp_dir):
...
class TestSafetensorsPerformance:
    ...
    def test_storage_size_comparison(self, temp_dir, sample_weights):
        ...
        assert hdf5_info['total_weights'] == len(sample_weights)  # HDF5 tracks weight count
...
class TestSafetensorsSpecialCases:
    ...
    def test_dtype_edge_cases(self, safetensors_store):
        ...
        test_cases = [
            ...
            (np.int32, np.array([np.iinfo(np.int32).min, 0, np.iinfo(np.int32).max], dtype=np.int32)),
...
class TestSafetensorsCLIIntegration:
    ...
    def test_conversion_workflow_simulation(self, temp_dir):
        ...
        save_file(test_tensors, st_file, metadata={"format": "pytorch", "version": "1.0"})
        ...
        save_file(converted_tensors, converted_file, metadata={"source": "coral_converted"})
