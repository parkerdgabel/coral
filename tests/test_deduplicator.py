...
class TestDeduplicator:
    ...
    def test_exact_deduplication(self):
        ...
        weight1 = WeightTensor(data=data, metadata=WeightMetadata(
            name="weight1", shape=(10, 10), dtype=np.float32
        ))
        weight2 = WeightTensor(data=data.copy(), metadata=WeightMetadata(
            name="weight2", shape=(10, 10), dtype=np.float32
        ))
    ...
    def test_similar_deduplication(self):
        ...
        weight1 = WeightTensor(data=data1, metadata=WeightMetadata(
            name="weight1", shape=(10, 10), dtype=np.float32
        ))
        weight2 = WeightTensor(data=data2, metadata=WeightMetadata(
            name="weight2", shape=(10, 10), dtype=np.float32
        ))
    ...
    def test_unique_weights(self):
        ...
        for i in range(5):
            ...
            weight = WeightTensor(data=data, metadata=WeightMetadata(
    ...
    def test_weight_groups(self):
        ...
        ref_weight = WeightTensor(data=ref_data, metadata=WeightMetadata(
            name="reference", shape=(10, 10), dtype=np.float32
        ))
        
        # Create duplicates and similar weights
        dup_weight = WeightTensor(data=ref_data.copy(), metadata=WeightMetadata(
            name="duplicate", shape=(10, 10), dtype=np.float32
        ))
        
        similar_data = ref_data + np.random.randn(10, 10).astype(np.float32) * 0.01
        similar_weight = WeightTensor(data=similar_data, metadata=WeightMetadata(
            name="similar", shape=(10, 10), dtype=np.float32
        ))
    ...
    def test_deduplication_report(self):
        ...
        # Add original
        dedup.add_weight(WeightTensor(data=base_data, metadata=WeightMetadata(
            name="original", shape=(100, 100), dtype=np.float32
        )))
        
        # Add duplicates
        for i in range(3):
            dedup.add_weight(WeightTensor(data=base_data.copy(), metadata=WeightMetadata(
        ...
        for i in range(2):
            ...
            dedup.add_weight(WeightTensor(data=similar, metadata=WeightMetadata(
