...
class Pruner:
    ...
    @staticmethod
    def _unstructured_magnitude_pruning(data: np.ndarray, sparsity: float) -> np.ndarray:
    ...
    def apply_mask(
        ...
        if mask.shape != weight.shape:
            raise ValueError(f"Mask shape {mask.shape} doesn't match weight shape {weight.shape}")
