...
class Quantizer:
    ...
    def quantize_uniform(
        ...
        if symmetric:
            ...
            # Quantize
            quantized = np.round(data / scale).astype(np.int8 if bits == 8 else np.int16)
            ...
            quantized = np.clip(quantized, qmin, qmax).astype(np.uint8 if bits == 8 else np.uint16)
        ...
        logger.debug(f"Quantized {weight.metadata.name} to {bits} bits "
    ...
    def dequantize(
        ...
        if symmetric:
            ...
            dequantized = (quantized_weight.data.astype(np.float32) - zero_point) * scale
