...
def main():
    ...
    print(f"Created {len(weights)} weights, total size: {total_data_size / 1024 / 1024:.2f} MB\n")
    ...
    configs = [
        ...
        (SafetensorsStore, {"use_compression": True, "compression_level": 6}, "SafeTensors (gzip)"),
    ]
    ...
    for store_class, kwargs, name in configs:
        ...
        print(f"  Compression ratio: {total_data_size / result['total_size_bytes']:.2f}x")
    
    # Summary comparison
    print("\n" + "=" * 60)
    print("SUMMARY COMPARISON")
    print("=" * 60)
    print(f"{'Backend':<30} {'Store (s)':<12} {'Load (s)':<12} {'Size (MB)':<12} {'Ratio':<8}")
    print("-" * 60)
    ...
    print(f"Fastest store: {fastest_store['name']} ({fastest_store['store_time']:.3f}s)")
    print(f"Fastest load: {fastest_load['name']} ({fastest_load['load_time']:.3f}s)")
    print(f"Smallest size: {smallest_size['name']} ({smallest_size['total_size_mb']:.2f} MB)")
    
    # SafeTensors vs HDF5 comparison
    hdf5_uncompressed = next(r for r in results if r["name"] == "HDF5 (no compression)")
    safetensors_uncompressed = next(r for r in results if r["name"] == "SafeTensors (no compression)")
    
    print(f"\nSafeTensors vs HDF5 (uncompressed):")
    print(f"  Store: {safetensors_uncompressed['store_time'] / hdf5_uncompressed['store_time']:.2f}x speed")
    print(f"  Load: {safetensors_uncompressed['load_time'] / hdf5_uncompressed['load_time']:.2f}x speed")
    print(f"  Size: {safetensors_uncompressed['total_size_bytes'] / hdf5_uncompressed['total_size_bytes']:.2f}x ratio")
