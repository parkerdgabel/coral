...
def main():
    ...
    try:
        ...
        variation = create_model_variation(base_model, similarity=0.995, 
        ...
        variation = create_model_variation(base_model, similarity=0.99,
        ...
        coral_size = storage_path.stat().st_size / (1024 * 1024) if storage_path.exists() else 0.0
        ...
        print(f"  - Deduplicated: {dedup_stats.total_weights - dedup_stats.unique_weights}")
        ...
        if base_weight and variation_weight:
            similarity = np.dot(base_weight.data.flatten(), variation_weight.data.flatten()) / (
                np.linalg.norm(base_weight.data.flatten()) * np.linalg.norm(variation_weight.data.flatten())
            )
